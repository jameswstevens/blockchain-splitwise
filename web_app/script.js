...
async function getNeighbors(user) {
	...
	var cur_accounts = await getUsers();
	if (cur_accounts.length > 0) {
		accounts = cur_accounts;
	}
	for (const account of accounts) {
		// if the account is either the debter or creditor to the user add to the neighbors
		if (await BlockchainSplitwise.connect(provider.getSigner(user)).lookup(user, account)) {
			neighbors.push(account);
		}
		if (await BlockchainSplitwise.connect(provider.getSigner(account)).lookup(account, user)) {
			neighbors.push(account);
		}
	}
	return neighbors;
...
async function getUsers() {
	var allFunctionCalls = await getAllFunctionCalls(contractAddress, "add_IOU");
	var allUsers = [];
	for (var i = 0; i < allFunctionCalls.length; i++) {
		var creditor = allFunctionCalls[i].args[0];
		var debtor = allFunctionCalls[i].from;
		if (!allUsers.includes(creditor)) {
			allUsers.push(creditor);
		}
		if (!allUsers.includes(debtor)) {
			allUsers.push(debtor);
		}
	}
	return allUsers;
}
...
async function getTotalOwed(user) {
	...
	var totalOwed = 0;
	for (const account of accounts) {
		var amount = await BlockchainSplitwise.connect(provider.getSigner(user)).lookup(user, account);
		totalOwed += amount;
	}
	return totalOwed;
}
...
async function getLastActive(user) {
	...
	for (var i = 0; i < allFunctionCalls.length; i++) {
		var creditor = allFunctionCalls[i].args[0];
		var debtor = allFunctionCalls[i].from;
		if (user.toLowerCase() === creditor.toLowerCase() || user.toLowerCase() === debtor.toLowerCase()) {
			var time = allFunctionCalls[i].t;
			if (lastTx === null || time > lastTx) {
				lastTx = time;
			}
		}
	}
...
async function add_IOU(creditor, amount) {
	addresses = await doBFS(creditor, defaultAccount, getNeighbors);
	var smallest_amount = 0;
	if (addresses !== null) {
		for (let i = 0; i < addresses.length - 1; i++) {
			var amount_owed = await BlockchainSplitwise.connect(provider.getSigner(defaultAccount)).lookup(addresses[i], addresses[i+1]);
			if (smallest_amount === 0 || amount_owed < smallest_amount) {
				smallest_amount = amount_owed;
			}
		}
		...
		for (let i = 0; i < addresses.length - 1; i++) {
			await BlockchainSplitwise.connect(provider.getSigner(addresses[i+1])).settle_up(addresses[i], smallest_amount);
		}
	}
	await BlockchainSplitwise.connect(provider.getSigner(defaultAccount)).add_IOU(creditor, amount - smallest_amount);
}
